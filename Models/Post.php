...
class Post
{
    ...
    */
    public static function GetPost($userID, $postID, $browsingUserID = null)
    {
            ...
            WHERE p.postID = {$postID}")) 
        {
            ...
            $row->timeAgo = Helper::TimeAgo($row->postDate);
    ...
    * Send post
    */
    public static function SendPost($userID, $postImage, $postTitle, $postKeywords = "")
    {
        $response = new SingleItemResponse();
        $mysqli = new mysqli(API_DB_SERVER, API_DB_USER, API_DB_PASS, API_DB_NAME);
        $userID = $mysqli->real_escape_string($userID);
        ...
        $sql = "INSERT INTO posts(userID, postImage, postTitle, postKeywords) VALUES ({$userID}, '{$postImage}', '{$postTitle}', '{$postKeywords}')";
        if($mysqli -> query($sql)) 
        {
            $response->status = 200;
